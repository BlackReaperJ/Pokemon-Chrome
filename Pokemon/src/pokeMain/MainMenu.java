...
public class MainMenu extends JFrame implements ActionListener{
	...
	Image imgBG, imgDialog, imgTitleScreen, imgProfessor, imgPokeball, imgTykepol, imgArrow, imgFriendGirl, imgFriendBoy, imgBoy, imgGirl,imgMainMenu, imgProfessorFlip, imgMenuAll, imgMenuContinue, imgMenuSelect, imgMenuAllSelect, imgPlayerMale, imgPlayerFemale, imgGender, imgCredits;
	...
	public static void main(String[ ] args) throws Exception{
		...
		musicPlayPoke creditMusic = new musicPlayPoke("Music/Credits.wav");
		musicPlayPoke introMusic = new musicPlayPoke("Music/PokemonHGSS.wav");

		while (true){   
			if (musicPlaying2 && (screenCount == 13 || screenCount == 14)){
				musicPlaying2 = false;
				introMusic.stop();
				introMusic = new musicPlayPoke("Music/PokemonHGSS.wav");
			}
			...
			if (screenCount != 14 && musicPlaying){
				musicPlaying = false;
				creditMusic.stop();
				creditMusic = new musicPlayPoke("Music/Credits.wav");
			}
			if (screenCount ==14) {
				if (!musicPlaying){
					musicPlaying = true;
			...
			//      if(GameFile.iBattle >=1 && GameFile.iLocation >=2 && !(GameFile.strPlayerName.equals("null"))){
			//          FakeBattle go = new FakeBattle();
			//        }s
			if (screenCount == 15){
				if(GameFile.iLocation >=0 &&  GameFile.iLocation <= 2 && GameFile.iaGameVars[3] < 12){
	...
	class MyDrawPanel extends JPanel {
		public void paintComponent(Graphics g) {
			...
			if (screenCount>=1){
				...
				if (screenCount==2){
					g.drawImage(imgMainMenu,0,0,600,600,0,0,252,192, this);
					if (selected==1){
						g.drawImage(imgMenuSelect,75,50,525,325,0,0,443,241, this);
						g.drawImage(imgMenuAll,75,350,525,425,0,0,443,66, this);
						g.drawImage(imgMenuAll,75,450,525,525,0,0,443,66, this);
					}
					if (selected==2){
						g.drawImage(imgMenuContinue,75,50,525,325,0,0,443,241, this);
						g.drawImage(imgMenuAllSelect,75,350,525,425,0,0,443,66, this);
						g.drawImage(imgMenuAll,75,450,525,525,0,0,443,66, this);
					}
					if (selected==3){
						g.drawImage(imgMenuContinue,75,50,525,325,0,0,443,241, this);
						g.drawImage(imgMenuAll,75,350,525,425,0,0,443,66, this);
						g.drawImage(imgMenuAllSelect,75,450,525,525,0,0,443,66, this);
					}
					g.setFont(f1);
					...
					if (gender==1)
						g.drawImage(imgPlayerMale,125,113,200,212,0,0,75,100, this);
					if (gender==2)
						g.drawImage(imgPlayerFemale,125,113,200,212,0,0,75,100, this);
				}
				if (screenCount>=3 && screenCount <=12){
					...
					if (screenCount>=3){
						...
						if (screenCount==3){
							...
							g.drawString("Hello, My Name is Professor Shinwa", 25,525);
							g.drawString("And this is the Orpheus Region!", 25,565);
							g.drawImage(imgPokeball,150,100, this);
						}
						else if(screenCount==4){
							g.drawImage(imgProfessorFlip,150,0, this);
							g.drawString("This creature is known as a Pokemon", 25,525);
							...
							g.drawImage(imgProfessorFlip,150,0, this);
							g.drawString("So, your name is " +GameFile.strPlayerName, 25,525);
							g.drawString("Well " +GameFile.strPlayerName+"........", 25,565);
						}
							...
							g.drawImage(imgProfessor,150,0, this);
							g.drawString("So, you're a "+ playerGenderString[finalPlayerGender], 25,525);
							...
							g.drawString("She is a spoiled rich girl who visits you", 25,565);
						}
						else if(screenCount==11){
							g.drawImage(imgProfessorFlip,150,0, this);
							g.drawImage(imgFriendBoy,400,0,this);
							g.drawString("And this is your other friend Jayce", 25,525);
							g.drawString("He is a sarcastic douchebag of a friend!", 25,565);
						}
						else if(screenCount ==12){
							g.drawImage(imgProfessor,150,0, this);
							g.drawString("Lets Start Your Adventure Now!", 25,525);
							g.drawString("To Become an Orpheus Pokemon Master!", 25,565);
						}
					}
				}
				if(screenCount ==14){
					...
					g.setColor(Color.BLUE);
					g.drawString("User-Interface and Battle Programmer", 70, 1800 -y);
					g.setColor(Color.WHITE);
					g.drawString("Julius D'Silva", 215,1850 -y);
					g.drawString("Dixon Dcuhna", 215,1900 -y);
					g.setColor(Color.BLUE);
					...
					g.setColor(Color.WHITE);
					g.drawString("Dixon Dcunha(Programmer)", 100,2750 -y);
					g.drawString("Justin Baradi(Programmer)", 100,2800 -y);
					g.drawString("Mudassir Mohammad(Pokemon Designer)", 95,2850 -y);
					g.drawString("Gabriel Ramos(Graphic Helper)", 100,2900 -y);
					g.drawString("Nidrax(Maple Trees)",100,2950 -y);
					g.drawString("XDinky(Fountain)",100,3000 -y);
					g.drawString("LotusKing(PokemonCenter)",100,3050 -y);
					g.drawString("chimcharsfireworkd(PokemonLab)",100,3100 -y);
					g.drawString("Eli-eli76(Battle Backgrounds)",100,3150 -y);
