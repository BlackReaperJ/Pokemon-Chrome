...
public class BoundaryBox {
	...
	public BoundaryBox(int xBound1, int xBound2, int yBound1, int yBound2,
			boolean preventsMovement, float chanceOfEncounter) {
		this(xBound1, xBound2, yBound1, yBound2, preventsMovement, null,
				chanceOfEncounter);

	}

	public BoundaryBox(int xBound1, int xBound2, int yBound1, int yBound2,
			boolean preventsMovement, Direction oneWayDirection) {

		this(xBound1, xBound2, yBound1, yBound2, preventsMovement,
				oneWayDirection, 0.0f);
	...
	public boolean isMovePossible(int currentX, int currentY,
		...
		if (directionOfMovement == Direction.RIGHT) {
			if (isIntBetween(currentY, y1, y2)) {
				if (isIntBetween(x1, currentX - pixelsMovedPerMoveAction,
						currentX)) {
					return false;
				}
				if (isIntBetween(x2, currentX - pixelsMovedPerMoveAction,
						currentX)) {
					return false;
				}
				if (isIntBetween(currentX, x1, x2)) {
			...
			if (isIntBetween(currentY, y1, y2)) {
				if (isIntBetween(x1, currentX, currentX
						+ pixelsMovedPerMoveAction)) {
					return false;
				}
				if (isIntBetween(x2, currentX, currentX
						+ pixelsMovedPerMoveAction)) {
					return false;
				}
				if (isIntBetween(currentX, x1, x2)) {
			...
			if (isIntBetween(currentX, x1, x2)) {
				if (isIntBetween(y1, currentY - pixelsMovedPerMoveAction,
						currentY)) {
					return false;
				}
				if (isIntBetween(y2, currentY - pixelsMovedPerMoveAction,
						currentY)) {
					return false;
				}
				if (isIntBetween(currentY, y1, y2)) {
			...
			if (isIntBetween(currentX, x1, x2)) {
				if (isIntBetween(y1, currentY, currentY
						+ pixelsMovedPerMoveAction)) {
					return false;
				}
				if (isIntBetween(y2, currentY, currentY
						+ pixelsMovedPerMoveAction)) {
					return false;
				}
				if (isIntBetween(currentY, y1, y2)) {
	...
	private static boolean isIntBetween(int theInt, int lesserNum,
			int greaterNum) {
		return (theInt >= lesserNum && theInt <= greaterNum);
	}

	public static boolean isMovePossible(int currentX, int currentY,
			Direction directionOfMovement, BoundaryBox[] boundaries) {

		for (BoundaryBox theBoundary : boundaries) {
			if (!theBoundary.isMovePossible(currentX, currentY,
					directionOfMovement)) {
				return false;
			}
		}
